...
def ensure_venv(target):
    ...
    print('Ensuring wheel ...')
    cmd('{target}/bin/pip install wheel'.format(target=target))
...
def ensure_minimal_python():
    ...
    for version in preferences[0:1]:
        ...
        print(
            'make Python {} available on this system.'.format(preferences[0])
        )
        sys.exit(66)
...
class AppEnv(object):
    ...
    def prepare(self, args=None, remaining=None):
        ...
        whitelist = {env_dir, os.path.join(self.appenv_dir, 'unclean')}
